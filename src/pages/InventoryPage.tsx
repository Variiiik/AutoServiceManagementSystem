...
const formatEUR = (v: number) => `${(v ?? 0).toFixed(2)} EUR`;
...
export const InventoryPage: React.FC = () => {
  ...
  const fetchInventory = async () => {
    try {
      ...
      const itemsWithLow = response.data.map((item: any) => ({
        ...item,
        is_low_stock:
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowModal(false);
      setEditingItem(null);
      setFormData({ name: '', sku: '', stock_quantity: '', min_stock_level: '', price: '' });
      fetchInventory();
    } catch (error) {
  ...
  // ⬇️ Oluline: ära tee parseInt – jäta UUID stringiks
  ...
  const lowStockItems = items.filter((i: any) => i.is_low_stock);
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
            ...
            <tbody className="bg-white dark:bg-slate-900 divide-y divide-gray-200 dark:divide-gray-800">
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={() => updateStock(String(item.id), Math.max(0, Number(item.stock_quantity) - 1))}
                        className="w-6 h-6 flex items-center justify-center bg-red-100 text-red-600 rounded hover:bg-red-200 disabled:opacity-50 dark:bg-red-900/30 dark:text-red-300 dark:hover:bg-red-900/50"
                      ...
                      <button
                        onClick={() => updateStock(String(item.id), Number(item.stock_quantity) + 1)}
                        className="w-6 h-6 flex items-center justify-center bg-green-100 text-green-600 rounded hover:bg-green-200 disabled:opacity-50 dark:bg-green-900/30 dark:text-green-300 dark:hover:bg-green-900/50"
                        disabled={user?.role !== 'admin'}
              ...
              {filteredItems.length === 0 && (
                <tr>
                  <td colSpan={user?.role === 'admin' ? 6 : 5} className="px-6 py-10 text-center text-gray-500 dark:text-gray-400">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white dark:bg-slate-900 rounded-lg p-6 w-full max-w-md border border-gray-200 dark:border-gray-800">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Price (EUR)</label>
                <input
                  type="number"
                  step="0.01"
                  min="0"
